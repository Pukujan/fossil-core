...
## Final retrieval security gate and campaign closeout — 2026-08-31

The final security gate passed from implementation SHA `7e69bdb955b74e7bfadb71b8ebf6cdbab77a295f`. The caller visibility policy filters before all BM25/D021/hybrid/reranked index construction and applies fail-closed defense in depth after retrieval. Direct canonical reads, citation authorization, exports, answer/context construction, receipts, disposable projection inspection, and projection rebuild were also exercised. The 9-document fixture and 8 adversarial cases produced 32 route/case observations; all 32 unfiltered probes returned attractive denied candidates, while all caller-filtered outputs had zero unauthorized IDs/text/citations, reranker ACL promotions, cross-pack results, suppression/redaction resurfacing, projection rebuild leaks, or receipt/export leaks. Four receipts validated with sidecar SHA-256 `d18744f61a8d338701a2db4ca083a6493a5392a6889e0daf93c21232d28c559e`.

Final decisions are `representation=RAW`, `embedding=D021`, `lexical=BM25`, `fusion=deterministic hybrid/RRF`, `reranker=retained pinned cross-encoder`, `normal_policy=RAW_RERANKED`, `routing=REJECT_ROUTING`, `llm_planner=DISABLED`, `contextual_enrichment=RETAIN_RAW/not promoted`, `graphiti=RETAIN_PROJECTION_NOT_RETRIEVAL`, `embedding_ladder=STOP_MODEL_LADDER`, and `security_boundary=PASS`. #47 and #48 are closed after reconciliation; #94 has the final `DONE` evidence. The post-Gate-2 RAG campaign is complete and has no next RAG experiment.
